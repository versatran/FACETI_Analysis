%nvn_load_data
%if you get an error running this script, take a look at
%image_processing_common_matter
% Get prefix - if not remote, there is no prefix.
function [prefix,data,num_stacks,image_struc,num_images,stack_text,func_name,...
...
%point to folder to find the files for 2013 as well.
data_path=[source_dir server_str expt_str filesep date_str(1:4) filesep date_str filesep ...
    expt_str '_' dataset_str];
% load data
...
elseif strcmp(data.raw.metadata.settype,'scan')
    if strcmp(date_str(1:4),'2014')
        ...
        metadata_struc.PV_scan_list=linspace(metadata_struc.Control_PV_start,...
            metadata_struc.Control_PV_end,metadata_struc.n_step);
...
%     separate_parts=find(func_name=='_');
    if length(find(bend_struc.func_name=='_'))>1        
        ...
        func_name=bend_struc.func_name(undescore_loc(1)+1:undescore_loc(2)-1);
    else
        func_name=bend_struc.func_name;
    end
    for i=1:num_stacks
        stack_text{i}=[func_name ': ' num2str(metadata_struc.PV_scan_list(i))];
    end    
    if strcmp(bend_struc.func_name,'set_QSBEND_energy')
        bend_struc.variable_bend=true;
        bend_struc.bend_values=data.raw.metadata.param.dat{1}.PV_scan_list;
        warning('This is a bend scan, dipole bend and zero GeV values will vary and need to be checked')
        for iii=1:length(bend_values)
            bend_struc.dipole_multiplier_values(iii)=(20.35+bend_values(iii))/20.35;
    ...
    error('You are running a new type of DAQ. Please write the code for it!');
end
